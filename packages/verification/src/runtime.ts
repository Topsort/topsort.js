...
/** Internal lifecycle seam; provider details are not exported from the package entrypoint. */
...
export function createVerificationRuntimeInternal(
  ...
  function beginProvider(record: Registration): void {
    ...
    let parsedTag: ParsedIasTag;
    try {
      parsedTag = deps.parseTag(record.tagIdentity, record.element.ownerDocument);
    } catch {
      terminate(record, "failed", "invalid_tag");
      return;
    }
    void startProvider(record, generation, parsedTag);
  }
